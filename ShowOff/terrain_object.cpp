...
terrain_object::terrain_object(int octaves, GLfloat freq, GLfloat scale, bool useTexture)
{
	attribute_v_coord = 0;
	attribute_v_colour = 1;
	attribute_v_normal = 2;
	attribute_v_texcoord = 3;
	attribute_v_lavatexcoord = 3;
...
terrain_object::~terrain_object()
{
	...
	if (lavaTexCoords) delete[] lavaTexCoords;
}
...
void terrain_object::createObject()
{
	...
	glBufferData(GL_ARRAY_BUFFER, xsize * zsize * sizeof(vec3), &(texCoords[0]), GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);

	/* Store the normals in a buffer object */
	glGenBuffers(1, &vbo_mesh_lavaTexCoords);
	glBindBuffer(GL_ARRAY_BUFFER, vbo_mesh_lavaTexCoords);
	glBufferData(GL_ARRAY_BUFFER, xsize * zsize * sizeof(vec3), &(lavaTexCoords[0]), GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
...
void terrain_object::drawObject(int drawmode)
{
	...
	if (enableTexture)
	{
		...
		glBindBuffer(GL_ARRAY_BUFFER, vbo_mesh_lavaTexCoords);
		glVertexAttribPointer(
			attribute_v_lavatexcoord, // attribute
			3,                  // number of elements per vertex, here (x,y,z)
			GL_FLOAT,           // the type of each element
			GL_FALSE,           // take our values as-is
			0,                  // no extra data between each position
		...
		glEnableVertexAttribArray(attribute_v_lavatexcoord);
	}
...
void terrain_object::createTerrain(GLuint xp, GLuint zp, GLfloat xs, GLfloat zs, GLfloat sealevel)
{
	...
	texCoords = new vec3[numvertices];
	lavaTexCoords = new vec3[numvertices];
	...
	for (GLuint row = 0; row < zsize; row++)
	{
		...
		for (GLuint col = 0; col < zsize; col++)
		{
			...
			//if (height <= 0.5 && height >= 0.385)
			lavaTexCoords[row * xsize + col] = vec3(xpos, (height - 0.5f) * height_scale, zpos);

			//if (height <= 0.385 && height >= 0)
			texCoords[row * xsize + col] = vec3(xpos, (height - 0.5f) * height_scale, zpos);
...
void terrain_object::setColourBasedOnHeight()
{
	...
	for (GLuint i = 0; i < numVertices; i++)
	{
		...
		if (height <= sea_norm)
			...
			//colours[i] = glm::vec3(1.0 + rand2, 1.0 + rand2, 1.0);
			colours[i] = glm::vec3(1.0, 1.0, 1.0);
		else if (height <= 0.52)// + rand3)
			//colours[i] = glm::vec3(0.847 + rand2, 0.847, 0.75);
			colours[i] = glm::vec3(0.847, 0.847, 0.75);
		else if (height <= 0.6)// + rand3)
			//colours[i] = glm::vec3(0.847, 0.847 + rand, 0.75);
			colours[i] = glm::vec3(0.847, 0.847, 0.75);
		else if (height <= 0.93)// + rand3)
			//colours[i] = glm::vec3(0.33 + rand2, 0.33, 0.33);
			colours[i] = glm::vec3(0.33, 0.33, 0.33);
		else
			//colours[i] = glm::vec3(0.89 + rand2, 0.47 + rand2, 0.20 + rand2);
			colours[i] = glm::vec3(0.89, 0.47, 0.20);
